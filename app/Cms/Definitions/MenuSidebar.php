...
use App\Models\MenuSidebar as MenuSidebarModel;
...
class MenuSidebar extends ResourceAdditionTree
{
    public $model = MenuSidebarModel::class;

    public $title = 'Меню Боковое';

    protected $cacheTag = 'menu_sidebar';

    public function fields()
    {
        return [
            Hidden::make('#', 'id'),
            ParentNodeMenuHeader::make('parent', 'parent_id')->default(request()->get('node') ?: (Session::get('MenuSidebar_node') ?: 1)),
    ...
    public function getList()
    {
        ...
        if (request()->get('node')) {
            Session::put('MenuSidebar_node', request()->get('node'));
        } else {
            $node = Session::get('MenuSidebar_node') ?: 1;
            header('Location: /admin/menu_sidebar?node='.$node);
        }
        //--
