...
class ProductsCodes extends ManyToManyAjax
{
    ...
    public function getOptionsSelected(Resource $definition)
    {
        if (request()->id) {
            ...
            */
            ...
            $selected = $definition->model()->find(request()->id)->blocksaleproducts()
                ->select(['product_code'])
                ->get();
            $result = [];
    ...
    public function save($collectionString, $model)
    {
        ...
        Cache::tags(['sales_product_'.$model->id, 'sales_'.$model->id])->flush();
    }
}
