...
class PromotionProductsCodes extends ManyToManyAjax
{
    ...
    public function getOptionsSelected(Resource $definition)
    {
        if (request()->id) {
            ...
            */
            ...
            $selected = $definition->model()->find(request()->id)->product_promotion()
                ->select(['product_code'])
                ->get();
            $result = [];
    ...
    public function save($collectionString, $model)
    {
        ...
        Cache::tags(['promo_product_'.$model->id, 'promo_'.$model->id])->flush();
    }
}
