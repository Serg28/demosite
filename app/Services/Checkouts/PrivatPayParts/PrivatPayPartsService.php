...
class PrivatPayPartsService
{
        ...
        if (empty($options) || ! is_array($options)) {
            ...
            foreach ($options as $PPOptions => $value) {
                if (method_exists($this, "set$PPOptions")) {
                    call_user_func_array([$this, "set$PPOptions"], [$value]);
                } else {
    ...
    public function create($method = 'pay')
    {
        if ($this->options['SUCCESS']) {
            ...
            if (! empty($this->recipientId)) {
                $param['recipient'] = ['recipientId' => $this->recipientId];
            }
     ...
     * @return mixed|string
     */
    public function getState(string $orderId = 'bar', $showRefund = true)
    {
    ...
    public function checkCallBack($string)
    {
        ...
        $sa = (is_array($string)) ? $string : json_decode($string, true);
     ...
     * @throws \InvalidArgumentException
     */
    public function setStoreId($argument)
    {
        if (empty($argument)) {
    ...
    {
        if (in_array($argument, ['II', 'PP', 'PB', 'IA'], false)) {
    ...
    {
        if (! empty($argument)) {
            if (in_array($argument, ['980'], false)) {
