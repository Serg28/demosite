...
abstract class AbstractFilter_orig
{
    use Referrer;
    ...
    public function maxPrice(int $price = 0): int
    {
        return $this->price['max'] ?? $price;
    }
    ...
    private function getCharacteristicOptionIds(array $options): array
    {
        return CharacteristicOption::slugIn($options)
            ->rememberForever()->cacheTags(['characteristicoptions'])
            ->pluck('id')->toArray();
    }
    ...
    private function getCharacteristic($category): ?Characteristic
    {
        return Characteristic::slug($category)
            ->rememberForever()->cacheTags(['characteristics'])
            ->first();
    }
    ...
    public function createUrl(array $filterOptions): string
    {
        $strUrlCollection = array_filter(array_map(
            static fn ($category, $options) => count($options) ? "$category=" . implode(',', $options) : null,
