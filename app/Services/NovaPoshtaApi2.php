...
class NovaPoshtaApi2
{
    ...
    {
        //--
        //$key = config('services.np.api_key');
        $key = setting('services.np.api_key') ?: config('services.np.api_key');
        if (! $key) {
            return;
        }
        //--
    ...
    {
        return $this->request('TrackingDocument', 'getStatusDocuments', ['Documents' => [['DocumentNumber' => $track, 'Phone' => $phone]]]);
    }
     ...
     * @return mixed
     */
    public function getWarehouse($cityRef, $description = '', $warehouse_ref = '')
    {
        ...
        if (is_array($warehouses['data'])) {
            if (count($warehouses['data']) == 1) {
                ...
                foreach ($warehouses['data'] as $warehouse) {
                    ...
                    or mb_stripos($warehouse['DescriptionRu'], $description) !== false or (! empty($warehouse_ref) && $warehouse['Ref'] == $warehouse_ref)) {
                        $data = $warehouse;
                        break;
                    }
                }
    ...
    public function getArea($findByString = '', $ref = '')
    {
        ...
        empty($this->areas) and $this->areas = include $_SERVER['DOCUMENT_ROOT'].'/assets/modules/evoshop/novaposhta/NovaPoshtaApi2Areas.php';
    ...
    public function getCity($cityName, $areaName = '')
    {
        ...
        if (is_array($cities['data'])) {
            ...
            $data = (count($cities['data']) > 1)
                ? $cities['data'][0]//$this->findCityByRegion($cities, $areaName)
                : $cities['data'][0];
        }
    ...
    public function newInternetDocument($sender, $recipient, $params)
    {
        ...
        if (! $sender['SenderAddress'] and $sender['CitySender'] and $sender['Warehouse']) {
            $senderWarehouse = $this->getWarehouse($sender['CitySender'], $sender['Warehouse'], $sender['WarehouseRef']);
        ...
        if (! @$recipient['RecipientAddress']) {
            $recipientWarehouse = $this->getWarehouse($recipient['CityRecipient'], $recipient['Warehouse'], $recipient['WarehouseRef']);
     ...
     * Update
     */
    public function updateDocument($sender, $recipient, $params)
    {
        // Check for required params and set defaults
        $this->checkInternetDocumentRecipient($recipient);
        $this->checkInternetDocumentParams($params);
