...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        foreach ($languages as $language) {
            DB::unprepared("
                CREATE TRIGGER before_tb_tree_{$language}_url_insert BEFORE INSERT ON tb_tree
                FOR EACH ROW
                BEGIN
            ...
            DB::unprepared("
                CREATE TRIGGER before_tb_tree_{$language}_url_update BEFORE UPDATE ON tb_tree
                FOR EACH ROW
                BEGIN
                    SET NEW.{$language}_url = JSON_UNQUOTE(JSON_EXTRACT(NEW.url, '$.\"$language\"'));
                END
    ...
    public function down()
    {
        ...
        /*$languages = ['ru', 'ua', 'en', 'pl']; // Замените на свои языки
        foreach ($languages as $language) {
            DB::unprepared("DROP TRIGGER IF EXISTS before_tb_tree_{$language}_url_insert");
            DB::unprepared("DROP TRIGGER IF EXISTS before_tb_tree_{$language}_url_update");
        }*/
