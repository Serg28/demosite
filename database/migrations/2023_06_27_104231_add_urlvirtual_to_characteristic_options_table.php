...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        foreach ($languages as $language) {
            DB::unprepared("
                CREATE TRIGGER before_characteristic_options_{$language}_url_insert BEFORE INSERT ON characteristic_options
                FOR EACH ROW
                BEGIN
            ...
            DB::unprepared("
                CREATE TRIGGER before_characteristic_options_{$language}_url_update BEFORE UPDATE ON characteristic_options
                FOR EACH ROW
                BEGIN
                    SET NEW.{$language}_url = JSON_UNQUOTE(JSON_EXTRACT(NEW.url, '$.\"$language\"'));
                END
    ...
    public function down()
    {
        ...
        /*$languages = ['ru', 'ua', 'en', 'pl']; // Замените на свои языки
        foreach ($languages as $language) {
            DB::unprepared("DROP TRIGGER IF EXISTS before_characteristic_options_{$language}_url_insert");
            DB::unprepared("DROP TRIGGER IF EXISTS before_characteristic_options_{$language}_url_update");
        }*/
