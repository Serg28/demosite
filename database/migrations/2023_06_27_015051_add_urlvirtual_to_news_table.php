...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        foreach ($languages as $language) {
            DB::unprepared("
                CREATE TRIGGER before_news_{$language}_url_insert BEFORE INSERT ON news
                FOR EACH ROW
                BEGIN
            ...
            DB::unprepared("
                CREATE TRIGGER before_news_{$language}_url_update BEFORE UPDATE ON news
                FOR EACH ROW
                BEGIN
                    SET NEW.{$language}_url = JSON_UNQUOTE(JSON_EXTRACT(NEW.url, '$.\"$language\"'));
                END
    ...
    public function down()
    {
        ...
        /*$languages = ['ru', 'ua', 'en', 'pl']; // Замените на свои языки
        foreach ($languages as $language) {
            DB::unprepared("DROP TRIGGER IF EXISTS before_news_{$language}_url_insert");
            DB::unprepared("DROP TRIGGER IF EXISTS before_news_{$language}_url_update");
        }*/
