...
//use App\Services\Basket as BasketService;
...
Route::group(
    ...
    function () {
        ...
        Route::group(['prefix' => 'payment'], function () {
            ...
            Route::post('/result/{order}', [PaymentController::class, 'result'])->name('payment.result.post')->withoutMiddleware(['firewall.all']);
        });
        ...
        //--
        Route::get('/promotions/{filter?}', [PromotionController::class, 'index'])->where('filter', '.*')->name('promotions');
        ...
        //Custom
        Route::get('/search', [SearchController::class, 'index'])->name('search-result.page');
        //Route::get('/search/{sortshow?}', [SearchControllerCustom::class, 'index'])->name('search')->where('sortshow', '.*');
        //

        Route::group(['prefix' => 'like'], function () { //, 'middleware' => 'auth.user'
            Route::post('/toggle/{product}', [LikeController::class, 'toggle'])->name('like.toggle');
            ...
            Route::post('/status', [LikeController::class, 'status'])->name('like.status');
        });
        ...
        Route::group(['prefix' => 'cart'], function () {
            Route::post('/add/{product}', [BasketController::class, 'addToCartById']);
        });
        ...
        Route::group(['prefix' => 'profile', 'middleware' => 'auth.login.home'], function () { //редирект на главную
            ...
            Route::get('/viewed-products', [ProfileController::class, 'viewedProducts'])->name('profile.viewed_products');
            Route::get('/cashback', [ProfileController::class, 'cashback'])->name('profile.cashback');
        ...
        Route::group(['prefix' => 'auth'], function () {
            ...
            Route::get('/activating-user/{user}/{token}', [AuthController::class, 'activatingUser'])->name('auth.activating_user');
            ...
            Route::get('/facebook/callback', [AuthSocialiteController::class, 'facebookCallback'])->name('auth.facebook.callback');

            Route::get('/google', [AuthSocialiteController::class, 'google'])->name('auth.google');
            Route::get('/google/callback', [AuthSocialiteController::class, 'googleCallback'])->name('auth.google.callback');
        });
...
Route::get('/{slug?}', [CategoryProductRouteController::class, 'route'])->where('slug', '^(?!.*\.(css|js|png|jpg|jpeg|gif|webp|svg|ico|pdf|doc|docx|xls|xlsx|ppt|pptx|zip|rar|tar|gz|html)$).*$')->name('catalog');
