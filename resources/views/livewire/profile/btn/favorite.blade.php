<div class="favorite-btn">
    @if (!empty(app('user')))
        <a href="{{route('profile.favorites')}}" class="favorite relative flex v--center h--center header-button">
            <span class="icon relative">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M16.7778 4C14.2982 4 12.8743 5.30695 12.2998 5.98929C12.1533 6.16341 11.8467 6.16341 11.7002 5.98929C11.1257 5.30695 9.70184 4 7.22222 4C4.37778 4 2 6.39342 2 9.23421C2 15.3185 10.6771 20.2769 11.8657 20.9276C11.952 20.9748 12.048 20.9748 12.1343 20.9276C13.3229 20.2769 22 15.3185 22 9.23421C22 6.39342 19.6222 4 16.7778 4Z" fill="#0A0527"/>
                </svg>
                @if(!empty($favoriteCount)) <span class="label absolute fsz-11 fw-500 flex v--center h--center">{{$favoriteCount}}</span> @endif
            </span>
        </a>
    @else
        <button class="favorite relative flex v--center h--center header-button js-lw-modal" data-component="auth.form.login">
            <span class="icon relative">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M16.7778 4C14.2982 4 12.8743 5.30695 12.2998 5.98929C12.1533 6.16341 11.8467 6.16341 11.7002 5.98929C11.1257 5.30695 9.70184 4 7.22222 4C4.37778 4 2 6.39342 2 9.23421C2 15.3185 10.6771 20.2769 11.8657 20.9276C11.952 20.9748 12.048 20.9748 12.1343 20.9276C13.3229 20.2769 22 15.3185 22 9.23421C22 6.39342 19.6222 4 16.7778 4Z" fill="#0A0527"/>
                </svg>
            </span>
        </button>
    @endif
</div>
