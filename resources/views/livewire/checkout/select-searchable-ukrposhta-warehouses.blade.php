<div>
    <div x-data="{ city_id: @entangle('cityId'), value: @entangle('value'), delivery_id: @entangle('deliveryId')}" class="select-searchable">
        <div wire:ignore :class="value ? '' : 'error'">
            <select class="select-choices select2-np-ukrposhta-{{md5($model)}}" wire:model.live="value"
...
<script>
    setTimeout(() => {
        ...
        SelectSearchable(window.lang + 'checkout/delivery/all-pointers', 'select2-np-ukrposhta-{{md5($model)}}', {}, {}, customRequestBody);
    }, 300);
</script>
@endscript