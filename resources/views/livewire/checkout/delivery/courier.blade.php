<div class="space-y-3">
    ...
    <div class="flex gap-4">
        <label class="flex items-center gap-2 cursor-pointer">
            <input wire:model.live="isElevator" type="checkbox" class="accent-brand">
            <span class="text-sm">{{ __t('Є ліфт') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            <input wire:model.live="isLifting" type="checkbox" class="accent-brand">
            <span class="text-sm">{{ __t('Потрібний підйом') }}</span>
        </label>
    </div>
</div>
