<div>
    <div x-data="{ city_id: @entangle('cityId'), value: @entangle('value')}" class="select-searchable">
        <div wire:ignore :class="value ? '' : 'error'">
            <select class="select-choices select2-np-street-{{md5($model)}}" wire:model="value"
...
<script>
    setTimeout(() => {
        ...
        SelectSearchable(window.lang + 'checkout/delivery/street-np', 'select2-np-street-{{md5($model)}}', {}, {}, customRequestBody);
    }, 300);
</script>
@endscript