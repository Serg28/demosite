@if (isset($page) && $page)
    <title>{{$page->getSeoTitle()}}@if(isset($currentCategory)), {!! str_replace('[category]', $currentCategory->t('title'), __t('категория «[category]»')) !!}@endif</title>
    ...
    @endif
    <meta property="og:title" content="{{$page->getSeoTitle($filter)}}@if(isset($currentCategory)), {!! str_replace('[category]', $currentCategory->t('title'), __t('категория «[category]»')) !!}@endif">
    ...
    @if (($page->seo && $page->seo->is_seo_noindex) || $filter->getCountFilterSelected() > 1 || empty($count))
