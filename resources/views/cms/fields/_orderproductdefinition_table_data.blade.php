...
<script>
	...
	function generateCostOrder() {
        ...
        $('[name=cost_without_sale]').val(costWithoutSale);

        if (sale) {
            priceSale = costWithoutSale * sale/100;
        }

        $('[name=cost]').val(Math.round(costWithoutSale - priceSale));
    }

    generateCostOrder();*/

    $('[name=sale],[name=sale_promo],[name=sale_discount]').blur(function () {
        generateCostOrder();
    });
    ...
    function generateCostOrder() {
        ...
        if (sale_promo || sale_discount) {
            //priceSale = costWithoutSale * (sale_promo+sale_discount)/100;
            //--
            var promoSaleSum = (sale_promo) ? Math.round((costWithoutSale * sale_promo) / 100) : 0;
            var discountSaleSum = (sale_discount) ? Math.round((costWithoutSale * sale_discount) / 100) : 0;
            priceSale = promoSaleSum+discountSaleSum;
            //--
