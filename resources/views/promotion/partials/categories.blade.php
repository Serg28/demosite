@php
    $activeCharacteristics = $page->characteristicsActive();
    $selected_filters = array_keys($filter->getSelectedFilters());
    $currentQuery = http_build_query(request()->except('page'));
    ...
    <div class="hidden-block">
        @if($categories)
            <div class="category-block">
                <ul class="filters_category">
                    @foreach($categories as $category)
                        <li>
                            <a
                                @if(@$currentCategory->id!==$category->id)
                                    href="{{geturl($page->getUrl())}}/category={{$category->id}}{{$currentQuery}}"
                                @else
                                    class="active"
                                @endif
                            >
        ...
        @loop($activeCharacteristics as $characteristicsItem)
                ...
                <div class="filter-wrapper category-block">
                    <div class="filters_category top block_has_li has_label">
                        ...
                        @foreach ($options as $option)
                            @php
                                $optionResults = $results['options'][$option->id] ?? 0;
                            @endphp
                            ...
                            <label for="{{$option->localizedSlug}}">
                                ...
                                <a @if (isset($results['options'][$option->id])) href="{{$filter->urlFilter($option)}}{{$currentQuery}}"
                                   @else class="disabled" @endif >
                                    {{$option->t('title')}} <span>({{$optionResults}})
                                </a>
                            </label>
                        @endforeach
                    </div>
                    ...
                    <div class="deployment-row ">
                        <a href="" class="d-flex ai-c open-filter">
                            <span><img src="/img/a1.svg" alt=""></span> <b data-text1='{{__t('Розгорнути')}}' data-text2='{{__t('Згорнути')}}'></b></a>
                    </div>
                    @endif
                </div>
            </div>
