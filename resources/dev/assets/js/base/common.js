...
event('.input .droppdown .dwoppdown-row','click',function(selector){
  let selectedText = selector.text();
  selector.parents('.input').findIn('input').val(selectedText);
})
...
event('.product-card','mouseover',function(selector,event){
  let hidden_block = selector.findIn('.hidden-block');
  if(hidden_block.length > 0){
    let height = hidden_block.position().height
...
event('.get-filter', 'click', function(selector) {
  el('html').toggleClass('hidden');
  el('.catalog-side-bar').toggleClass('active');
})

event('.filter-closer', 'click', function(selector) {
  el('html').removeClass('hidden');
  el('.catalog-side-bar').removeClass('active');
})
//--
...
event('.scroll-to', 'click', function(selector, e){
...
event('.scroll-to-seo', 'click', function(selector, e){
  let parameter1 = el(selector.attr('href'))
  let parameter2 =  500
  let parameter3 = 100
  e.preventDefault();
...
if(el('.account-page__side-bar a.current').length>0){
...
if(el('.info-page-side-bar a.current').length>0){
...
window.onpopstate = function(event) {
    /* Перезагрузка страницы при использовании кнопки "Назад" */
